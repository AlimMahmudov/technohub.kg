...
const Discount = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (!scroll || discounted.length <= 3) return;

      const containerWidth = scroll.offsetWidth;
      const scrollStep = containerWidth / 3; // ширина одной карточки
      const maxScrollLeft = scroll.scrollWidth - containerWidth;

      const nextScrollLeft = scroll.scrollLeft + scrollStep;
