          ...
          {discount.map((el, index) => (
            <Link key={index} href={`/detail/${el.id}`}>
              <div className="bg-white flex  flex-col gap-3 justify-between rounded-[10px] border border-gray-200 p-3 shadow-md">
                <div className=" flex justify-start w-full h-[240px] rounded-[10px] overflow-hidden">
                  <Image
                    className="object-cover w-full h-full"
                    src={el.image}
                    alt="product"
                  />
                  <div className="absolute mt-2 ml-2 px-1 rounded-[10px] bg-red-500 text-white text-[18px] font-[600]">
